...
def train_knowledge_distillation(teacher, student, X_train, y_train, X_valid, y_valid, X_test, y_test, epochs, learning_rate, T, soft_target_loss_weight, ce_loss_weight, device, model_name="", path_log_base="logs", batch_size=32):
    start_time = tm.time()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    student.to(device)
